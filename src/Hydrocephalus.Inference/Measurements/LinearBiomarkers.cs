...
public static class LinearBiomarkers
{
    ...
    ///
    /// Верхняя граница взята по опубликованным данным, а не по геометрии.
    /// Развёрнутый угол 180° геометрически возможен, но анатомически нет:
    /// у контролей 112 ± 11° [Ishii 2008], по упрощённой методике 138,5 ± 5,2°,
    /// при иНТГ 66 ± 14°. Весь разброс укладывается в 30–150.
    ///
    /// Граница нужна: автоматическое измерение дало на клинической серии
    /// 163,7° и 170,2° в двух прогонах — крыши желудочков там оказались
    /// плоскими, и число, лежащее внутри прежнего диапазона, ничем не
    /// отличалось от настоящего. Проверки формы для этого случая найти не
    /// удалось: величина наклона крыши однозначно выражается через сам угол,
    /// то есть говорит о нём же и ничего не добавляет.
    ///
    /// Это по-прежнему проверка правдоподобия измерения, а не признак болезни:
    /// диагностический порог 90° (среднее контролей минус два стандартных
    /// отклонения) принадлежит модели и протоколу валидации, а не измерителю.
    /// </summary>
    private static readonly MeasurementRange CallosalAnglePlausibleRange = new(30.0, 150.0);
