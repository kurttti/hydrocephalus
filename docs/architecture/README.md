...
События аудита: импорт, результат QC, запуск/отмена анализа, версия модели, предпросмотр и экспорт отчёта (с вариантом), экспорт манифеста датасета, уборка рабочих копий (с числами, включая неудавшиеся удаления), отказ по правам (с инициатором) и ошибка. Журнал читается в приложении только с правом `ReadAuditLog` (есть у администратора) и всегда вместе с проверкой цепочки хешей; пути профиля на экране заменены переменными окружения, потому что содержат имя учётной записи. Запрещено записывать пиксельные данные и исходные DICOM-теги в логи. Техническая телеметрия выключена по умолчанию.
