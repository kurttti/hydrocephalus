...
def measure_callosal_angle(
    ...
    # Крыши должны образовывать пик у средней линии: левая идёт вверх к
    # центру, правая — вниз от него. Плоская или односкатная крыша даёт угол
    # около 180°, который внутри правдоподобного диапазона 30–180 ничем не
    # отличается от настоящего.
    #
    # Раньше здесь стояла проверка связности маски, и она браковала годные
    # серии: перегородка между желудочками тоньше вокселя, они соприкасаются,
    # и связность не отличает настоящее слияние от касания.
    if left_slope <= 0.0 or right_slope >= 0.0:
        raise ValueError(
            f"Крыши не сходятся в пик у средней линии (наклоны {left_slope:+.2f} "
            f"и {right_slope:+.2f}): угол между ними не определён."
        )
